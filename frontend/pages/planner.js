...
const $$PlanEditor = ({parent, plan, dayDetails, onConfirm, onCancel, onDelete}={})=>({ div: {

  props: {
    parent: parent, 
    planNotes: plan.project.notes,
  },
  ...
  '=>':[
    ...
    { div: {
      ...
      '=>': [
        ...
        { h5: { text: "Notes - " + dayDetails }},
        // { textarea: { 
        //   style: "min-height: 100px",
        //   'ha.value': Bind($ => $.scope.planNotes)
        // }},
        ...
        { div: {
          ...
          def: {
            load: $ =>  {
              $.editorInstance.setContents($.planNotes)
            },
            save:  $ =>  {
              $.planNotes = $.editorInstance.getContents()
            }
          },
          ...
          css: [".ql-container { height: calc(100% - 160px)}"]
        ...
        { button: { text: "Confirm", handle: { onclick: $=>{ $.ref.editor.save(); onConfirm({notes: $.scope.planNotes}) } }}}
      ]
    }}
...
const PlannedPlan = { div: { meta: {forEach:"plan", of: $=>$.scope.planned},
  ...
  a: { 
    style: $=>({
      ...
      backgroundColor: $.projects?.find(p=>p.id === $.meta.plan.project.id).color,
      borderRadius: "7px",
      textAlign: "center",
  ...
  handle: {
    ...
    onclick: ($, evt)=>{
      ...
      $.incrementOrDecrementPlan(isIncrement, 1)
    },
    oncontextmenu: ($, evt)=>{
      evt.stopPropagation();
      evt.preventDefault();
      $.incrementOrDecrementPlan(false, 1)
    }
  },

  def: {
    incrementOrDecrementPlan: ($, isIncrement, amount = 1) => {
      if (isIncrement){
    ...
    openPlanEditor: $=>{
      ...
      let onConfirm = (edits)=>{
        $.meta.plan.project.notes = edits.notes
        $.refreshPlan()
        app.destroy()
        app = undefined
      }
      ...
      app = RenderApp(document.body, $$PlanEditor({parent: $.this, plan: $.meta.plan, dayDetails: $.projects?.find(p=>p.id === $.meta.plan.project.id).name + " - " + $.day,  onConfirm: onConfirm, onCancel: onCancel, onDelete: onDelete}))
    }
  },
  ...
  text: [ 
    { div: { 
      ...
      text: $=>$.projects?.find(p=>p.id === $.meta.plan.project.id).name
    }},
    
    // INFO
    { div: { 
      a:{ style:"font-size: 9px; top: -2px; left: 2px; position: absolute; text-align: left; border: 0.25px solid #ffffff33; border-radius: 50px; padding: 0px 1px; cursor: pointer"},  
      handle: {
        onclick: ($, evt)=>{
            evt.stopPropagation()
            evt.preventDefault()
    ...
    // GRAB
    { div: { 
      style: "font-size: 9px;top: 3px;width: 50%;height: 4px; left: 25%;position: absolute;text-align: left;border: 2.5px solid #ffffff44; border-radius: 50px; padding: 0px 1px; cursor: grab"
    }},
    { div: { 
      style: "font-size: 9px;top: 3px;width: 50%;height: 4px; left: 25%;position: absolute;text-align: left;border: 6px solid #ffffff00; border-radius: 50px; padding: 0px 1px; cursor: grab"
    }},

    // DELETE PLAN
    { div: { 
      a:{ style:"font-size: 9px; top: -2px; right: 2px; position: absolute; text-align: right; border: 0.25px solid #ffffff33; border-radius: 50px; padding: 0px 1px; cursor: pointer"},  
      handle: {
        onclick: ($, evt)=>{
            evt.stopPropagation()
            evt.preventDefault()
    ...
    // HH --
    { div: { 
      a:{ style:"font-size: 9px; bottom: -2px; left: 2px; position: absolute; text-align: left; border: 0.25px solid #ffffff33; border-radius: 50px; padding: 0px 1px"},  
      handle: {
        onclick: ($, evt)=>{
            evt.stopPropagation()
            evt.preventDefault()
    ...
    // HH ++
    { div: { 
      a:{ style:"font-size: 9px; bottom: -2px; right: 2px; position: absolute; text-align: right; border: 0.25px solid #ffffff33; border-radius: 50px; padding: 0px 1px; cursor: pointer"},  
      handle: {
        onclick: ($, evt)=>{
            evt.stopPropagation()
            evt.preventDefault()
            console.log("PLAN ++")
            $.incrementOrDecrementPlan(true, 1)
        }}, 
      text: "+"
    }},
  ]
  ...
  '=>':[
    ...
    { div: {
      ...
      '=>': [
        ...
        { h5: { text: "Color" }},
        { input: { 
          'a.value': Bind($ => $.scope.projectColor)
        }},
        { input: { 
          'ha.type': 'color',
          'a.value': Bind($ => $.scope.projectColor)
        }},
        // { div: { text: "", a_style: $=>({backgroundColor: $.scope.projectColor, width: "40px", height: "20px"}) }},

        { h5: { text: "Notes" }},
        { textarea: { 
          style: "min-height: 100px",
          'ha.value': Bind($ => $.scope.projectNotes)
        }},
        ...
        { button: { text: "Cancel", handle: { onclick: $=>onCancel() }}},
        { button: { text: "Delete", handle: { onclick: $=>onDelete() }}},
        { button: { text: "Confirm", handle: { onclick: $=>onConfirm({name: $.scope.projectName, color: $.scope.projectColor, notes: $.scope.projectNotes}) }}}
      ]
    }}
...
const ProjectsToolbar = { div: { meta: {forEach: "project", of: $=>$.scope.projects},
  ...
  handle: {
    ondragstart: ($, evt)=>{
      $.scope.moovingPlan = { new_plan: true, plan: { project: { id: $.meta.project.id, notes: "" } } }
    },
    onclick: $=>{
...
export const PlannerPage = async (state)=>{ //let usr=prompt("User?", "shared"); 
return { 
  div: {
    id: "app",
    ...
    props: {
      ...
      plans: state !== undefined ? state.plans : {},//{
        // "2022-4-1":[
        //   { slot_idx: 0, len: 2, project: { id: "proj1", color: "#f1c40f"} },
        //   { slot_idx: 2, len: 2, project: { id: "proj2", color: "#1abc9c"} }
        // ], 
    ...
    onInit: $=>{
      if(state === undefined){
        ...
        try{
          ...
          $.this.projectUUID = 4,
          $.this.projects = [
            { id: "proj1", name: "Project 1", color: "#f1c40f", notes: ""},
            { id: "proj2", name: "Project 2", color: "#1abc9c", notes: ""},
            { id: "proj3", name: "Project 3", color: "#e74c3c", notes: ""},
            { id: "proj4", name: "Project 4", color: "#2ecc71", notes: ""}
          ],
          $.this.plans = {}
        }
    ...
    def: {
      ...
      createNewProject: $=>{
        $.this.projectUUID++
        $.this.projects = [...$.this.projects, { id: "proj"+$.this.projectUUID, name: "Project "+$.this.projectUUID, color: "#353535", notes: ""}]
      },

      editProject: ($, projectRef, edits)=>{
        let {name, color, notes} = edits
        projectRef.name = name
        projectRef.color = color
        projectRef.notes = notes
      ...
      storage: {
        save: $=>{
          // await $.le.api.planner.save({projectUUID: $.projectUUID, projects: $.projects, plans: $.plans}, usr)
        ...
        load: $=>{
          ...
          // let data = await $.le.api.planner.load(usr)

          // $.scope.projectUUID = data.projectUUID
          // $.scope.projects = dataprojects
        ...
        { div: { 
          ...
          "=>": [
            smart({button: "<"}, {handle: {onclick: $=>{
              ...
              Router.navigate("planner", {viewDay: previusMonth, plans: $.scope.plans, projects: $.scope.projects, projectUUID: $.scope.projectUUID})
            ...
            smart({button: ">"}, {handle: {onclick: $=>{
              ...
              Router.navigate("planner", {viewDay: nextMonth, plans: $.scope.plans, projects: $.scope.projects, projectUUID: $.scope.projectUUID})
            } }, 'a.style': { background: 'transparent', border: 'none', color: '#dddddd', cursor: 'pointer', fontSize: '1.9rem', fontWeight: 100} }),
          ]
        }}
      ],
